...
Start (){
    ...
    readonly PACKAGES=("iputils-ping" "lm-sensors" "htop" "network-manager" "net-tools" "cockpit" "cockpit-navigator" "realmd" "tuned" "udisks2-lvm2" "samba" "winbind" "nfs-kernel-server" "nfs-common" "cockpit-file-sharing" "wireguard-tools" "unattended-upgrades")
...
NFS_Mount(){
    ...
    if ping -c 1 "$NAS_IP" &> /dev/null; then
        ...
        if [[ $(findmnt -M "$WORK_DIR"/docker) ]]; then
            ...
            mount -t nfs "$NAS_IP":/volume1/Server "$WORK_DIR"/docker
            Check_Success $? "NAS NFS mount"
            Show 2 "Making the mount permanent"
            if ! grep ~/docker /etc/fstab; then
                echo "$NAS_IP:/volume1/Server $WORK_DIR/docker  nfs      defaults    0       0" >> /etc/fstab
